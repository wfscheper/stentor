...
func init() {
	...
	file2 := &embedded.EmbeddedFile{
		...
		FileModTime: time.Unix(1600137814, 0),
	...
	file3 := &embedded.EmbeddedFile{
		...
		FileModTime: time.Unix(1600137639, 0),

		Content: string("{{- /*\n    Copyright © 2020 The Stentor Authors\n    Licensed under the Apache License, Version 2.0 (the \"License\");\n    you may not use this file except in compliance with the License.\n    You may obtain a copy of the License at\n\n        http://www.apache.org/licenses/LICENSE-2.0\n\n    Unless required by applicable law or agreed to in writing, software\n    distributed under the License is distributed on an \"AS IS\" BASIS,\n    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.\n    See the License for the specific language governing permissions and\n    limitations under the License.\n*/ -}}\n{{- $repository := .Repository -}}\n{{- $sectionHeader := .SectionHeader -}}\n\n{{ .Header }} [{{ .Version }}] - {{ .Date.Format \"2006-01-02\" }}\n{{- range .Sections -}}\n{{- if or .Fragments .ShowAlways }}\n\n{{ $sectionHeader }} {{ .Title }}\n\n{{ range .Fragments -}}\n- {{ .Text | indent 2 }}{{ if .Issue }}\n  [#{{ .Issue }}](https://github.com/{{ $repository }}/issues/{{ .Issue }}){{ end }}\n{{ else -}}\n{{ if .ShowAlways -}}\nNo significant changes.\n{{ end -}}\n{{ end -}}\n{{ end -}}\n{{- end }}\n\n[{{ .Version }}]: https://github.com/{{ .Repository }}/compare/{{ .PreviousVersion }}...{{ .Version }}\n\n\n----\n\n"),
	}
	file4 := &embedded.EmbeddedFile{
		Filename:    "github-rst-header",
		FileModTime: time.Unix(1600137869, 0),
	...
	file5 := &embedded.EmbeddedFile{
		...
		FileModTime: time.Unix(1600137656, 0),

		Content: string("{{- /*\n    Copyright © 2020 The Stentor Authors\n    Licensed under the Apache License, Version 2.0 (the \"License\");\n    you may not use this file except in compliance with the License.\n    You may obtain a copy of the License at\n\n        http://www.apache.org/licenses/LICENSE-2.0\n\n    Unless required by applicable law or agreed to in writing, software\n    distributed under the License is distributed on an \"AS IS\" BASIS,\n    WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.\n    See the License for the specific language governing permissions and\n    limitations under the License.\n*/ -}}\n{{- $repository := .Repository -}}\n{{- $sectionHeader := .SectionHeader -}}\n{{- $date := .Date.Format \"2006-01-02\" -}}\n\n`{{ .Version }}`_ - {{ $date }}\n{{ .Header | repeat (sum (len .Version) (len $date) 6) }}\n{{- range .Sections -}}\n{{- if or .Fragments .ShowAlways }}\n\n{{ .Title }}\n{{ $sectionHeader | repeat (len .Title) }}\n\n{{ range .Fragments -}}\n- {{ .Text | indent 2 }}{{ if .Issue }}\n  `#{{ .Issue }} <https://github.com/{{ $repository }}/issues/{{ .Issue }}>`_{{ end }}\n{{ else -}}\n{{ if .ShowAlways -}}\nNo significant changes.\n{{ end -}}\n{{ end -}}\n{{ end -}}\n{{- end }}\n\n.. _{{ .Version }}: https://github.com/{{ .Repository }}/compare/{{ .PreviousVersion }}...{{ .Version }}\n\n\n----\n\n"),
	}

	// define dirs
	dir1 := &embedded.EmbeddedDir{
		Filename:   "",
		DirModTime: time.Unix(1594266234, 0),
